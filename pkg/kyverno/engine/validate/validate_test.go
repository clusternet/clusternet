...
func TestValidateMap(t *testing.T) {
	...
	path, err := validateMap(resource, pattern, pattern, "/", anchor.NewAnchorMap())
	assert.Equal(t, path, "")
...
func TestValidateMapElement_TwoElementsInArrayOnePass(t *testing.T) {
	...
	path, err := validateResourceElement(resource, pattern, pattern, "/", anchor.NewAnchorMap())
	assert.Equal(t, path, "")
...
func TestValidateMap_CorrectRelativePathInConfig(t *testing.T) {
	...
	pattern, err := variables.SubstituteAll(nil, pattern)
	assert.NilError(t, err)
...
func testValidationPattern(t *testing.T, num string, patternBytes []byte, resourceBytes []byte, path string, nilErr bool) {
	...
	p, err := validateResourceElement(resource, pattern, pattern, "/", anchor.NewAnchorMap())
...
func testMatchPattern(t *testing.T, testCase struct {
	...
	err = MatchPattern(resource, pattern)
