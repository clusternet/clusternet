...
import (
	...
	"github.com/clusternet/clusternet/pkg/kyverno/engine/validate"
...
func preProcessPattern(pattern, resource *yaml.RNode) error {
	err := preProcessRecursive(pattern, resource)
	if err != nil {
		return err
	}
...
func preProcessRecursive(pattern, resource *yaml.RNode) error {
	switch pattern.YNode().Kind {
	case yaml.MappingNode:
		return walkMap(pattern, resource)
	case yaml.SequenceNode:
		return walkList(pattern, resource)
	}
...
func walkMap(pattern, resource *yaml.RNode) error {
	...
	if err := validateConditions(pattern, resource); err != nil {
	...
	for _, field := range nonAnchors {
		...
		err := preProcessRecursive(pattern.Field(field).Value, resourceValue)
		if err != nil {
			return err
		}
	}
...
func walkList(pattern, resource *yaml.RNode) error {
	...
	if elements[0].YNode().Kind == yaml.MappingNode {
		return processListOfMaps(pattern, resource)
	}
...
func processListOfMaps(pattern, resource *yaml.RNode) error {
	...
	for _, patternElement := range patternElements {
		...
		if hasAnyAnchor {
			...
			for _, resourceElement := range resourceElements {
				if err := preProcessRecursive(patternElementCopy, resourceElement); err != nil {
					klog.V(3).Info("anchor mismatch", "reason", err.Error())
			...
			if resource == nil {
				if err := preProcessRecursive(patternElementCopy, resource); err != nil {
					klog.V(3).Info("anchor mismatch", "reason", err.Error())
					if isConditionError(err) {
						continue
					}
...
func validateConditions(pattern, resource *yaml.RNode) error {
	var err error
	err = validateConditionsInternal(pattern, resource, anchor.IsGlobalAnchor)
	if err != nil {
		return NewGlobalConditionError(err)
	}

	err = validateConditionsInternal(pattern, resource, anchor.IsConditionAnchor)
	if err != nil {
...
func checkCondition(pattern *yaml.RNode, resource *yaml.RNode) error {
	...
	err = validate.MatchPattern(resourceInterface, patternInterface)
	if err != nil {
		return err
	}
...
func validateConditionsInternal(pattern, resource *yaml.RNode, filter func(string) bool) error {
	...
	for _, condition := range conditions {
		...
		if err := checkCondition(patternValue, resourceValue); err != nil {
			return err
		}
	}
