...
func NewAllInHandler(ctx context.EvalInterface) OperatorHandler {
	return AllInHandler{
		ctx: ctx,
	}
}
...
func (allin AllInHandler) validateValueWithStringPattern(key string, value interface{}) (keyExists bool) {
	invalidType, keyExists := allKeyExistsInArray(key, value)
	if invalidType {
...
func allKeyExistsInArray(key string, value interface{}) (invalidType bool, keyExists bool) {
...
func (allin AllInHandler) validateValueWithStringSetPattern(key []string, value interface{}) (keyExists bool) {
	invalidType, isAllIn := allSetExistsInArray(key, value, false)
	if invalidType {
		klog.V(2).Info("expected type []string", "value", value, "type", fmt.Sprintf("%T", value))
		return false
	}
...
func allSetExistsInArray(key []string, value interface{}, allNotIn bool) (invalidType bool, keyExists bool) {
	switch valuesAvailable := value.(type) {
	case []interface{}:
	...
	case string:
		...
		if operatorVariable == operator.InRange {
			if allNotIn {
				isAllNotInBool := true
				for _, k := range key {
					if handleRange(k, valuesAvailable) {
