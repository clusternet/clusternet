...
func NewAnyInHandler(ctx context.EvalInterface) OperatorHandler {
	return AnyInHandler{
		ctx: ctx,
	}
}
...
func anyKeyExistsInArray(key string, value interface{}) (invalidType bool, keyExists bool) {
	...
	case string:
		...
		if operatorVariable == operator.InRange {
			return false, handleRange(key, value)
		}
...
func handleRange(key string, value interface{}) bool {
	if !common.ValidateValueWithPattern(key, value) {
		return false
	} else {
		return true
	}
...
func (anyin AnyInHandler) validateValueWithStringSetPattern(key []string, value interface{}) (keyExists bool) {
	invalidType, isAnyIn := anySetExistsInArray(key, value, false)
	if invalidType {
		klog.V(2).Info("expected type []string", "value", value, "type", fmt.Sprintf("%T", value))
		return false
	}
...
func anySetExistsInArray(key []string, value interface{}, anyNotIn bool) (invalidType bool, keyExists bool) {
	switch valuesAvailable := value.(type) {
	case []interface{}:
	...
	case string:
		...
		if operatorVariable == operator.InRange {
			if anyNotIn {
				...
				for _, k := range key {
					if handleRange(k, stringForAnyNotIn) {
						isAnyNotInBool = true
						break
					}
				}
