...
// This file was copied from k8s.io/kubernetes/pkg/printers/tablegenerator.go
...
type TableGenerator interface {
	GenerateTable(obj k8sruntime.Object, options GenerateOptions) (*metav1.Table, error)
}

// PrintHandler - interface to handle printing provided an array of metav1.TableColumnDefinition
type PrintHandler interface {
	TableHandler(columns []metav1.TableColumnDefinition, printFunc interface{})
}
...
type HumanReadableGenerator struct {
	handlerMap map[string]*handlerEntry
}
...
func (h *HumanReadableGenerator) GenerateTable(obj k8sruntime.Object, options GenerateOptions) (*metav1.Table, error) {
	handler := h.handlerMap[obj.GetObjectKind().GroupVersionKind().Kind]
...
func (h *HumanReadableGenerator) TableHandler(columnDefinitions []metav1.TableColumnDefinition, printFunc interface{}) {
	...
	funcName := GetFunctionName(printFunc)
	resourceKind := strings.TrimPrefix(funcName, "print")

	if _, ok := h.handlerMap[resourceKind]; ok {
		err := fmt.Errorf("registered duplicate printer for %s", resourceKind)
		utilruntime.HandleError(err)
		return
	}
	h.handlerMap[resourceKind] = entry
}
