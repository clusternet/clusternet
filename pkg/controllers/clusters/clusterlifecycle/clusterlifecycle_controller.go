...
func (c *Controller) deleteCluster(obj interface{}) {
	...
	if !ok {
		...
		mcls, ok = tombstone.Obj.(*clusterapi.ManagedCluster)
		if !ok {
			utilruntime.HandleError(fmt.Errorf("tombstone contained object that is not a ManagedCluster %#v", obj))
			return
		}
	}
	klog.V(4).Infof("deleting ManagedCluster %q", klog.KObj(mcls))
	c.enqueue(mcls)
}
...
func (c *Controller) syncHandler(key string) (time.Duration, error) {
	...
	// requeue duration is a little longer than the duration we consider a cluster to be Unknown
	requeueAfter := DefaultClusterMonitorGracePeriod + 10*time.Second
	if mcls.Status.HeartbeatFrequencySeconds != nil {
		requeueAfter = time.Second*time.Duration(*mcls.Status.HeartbeatFrequencySeconds)*ClusterNotReadyThreshold + 10*time.Second
	}
...
func (c *Controller) updateClusterToLostIfNeeded(ctx context.Context, cluster *clusterapi.ManagedCluster) error {
	...
	observedReadyCondition := c.getClusterReadyCondition(cluster)
...
// getClusterReadyCondition return cluster unknown condition based on cluster status and grace period
func (c *Controller) getClusterReadyCondition(cluster *clusterapi.ManagedCluster) *metav1.Condition {
	var readyCondition *metav1.Condition
	var probeTimestamp metav1.Time

	gracePeriod := DefaultClusterMonitorGracePeriod
	if cluster.Status.HeartbeatFrequencySeconds != nil {
		gracePeriod = time.Second * time.Duration(*cluster.Status.HeartbeatFrequencySeconds) * ClusterNotReadyThreshold
	}
