...
const (
	...
	// ResourceEvent Update resource metadata or spec
	ObjectUpdateTemp = "UpdateTemp"
	...
	ri := utils.NewResourceInformer(resourceClient, apiResource, cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(oldObj, newObj interface{}) {
			...
			if !reflect.DeepEqual(oldspec, newspec) {
				resAttrs, err = getResourceAttrs(val, ObjectUpdateTemp)
			} else if !reflect.DeepEqual(oldstatus, newstatus) || (len(oldstatus) != len(newstatus)) {
			...
			if ok && err == nil {
				klog.V(5).Infof("updating %s %q", c.name, klog.KObj(oldresource))
				c.enqueue(resAttrs)
			}
		},
		DeleteFunc: func(obj interface{}) {
			...
			if ok && err == nil {
				klog.V(5).Infof("deleting %s %q", c.name, klog.KObj(resource))
				c.enqueue(resAttrs)
			}
		},
	})
