...
// FeedInventoryInformer provides access to a shared informer and lister for
// FeedInventories.
...
// NewFeedInventoryInformer constructs a new informer for FeedInventory type.
...
func NewFeedInventoryInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredFeedInventoryInformer(client, namespace, resyncPeriod, indexers, nil)
}

// NewFilteredFeedInventoryInformer constructs a new informer for FeedInventory type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AppsV1alpha1().FeedInventories(namespace).List(context.TODO(), options)
			},
...
func (f *feedInventoryInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredFeedInventoryInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}
